    ...
    def _generate_response(self, messages: list[Message]) -> str:
        ...
        if "classify" in last_content and "category" in last_content:
            ...
            if any(kw in user_msg for kw in ["deal", "validate", "rules"]):
                return "agent_task"
            if any(kw in user_msg for kw in ["conductor", "acsr", "aac", "pylon", "pylons", "span", "spans", "tower", "towers"]):
                return "agent_task"
            if any(kw in user_msg for kw in ["document", "search", "nerc", "standard", "specification"]):
                return "rag_query"
            return "general_chat"
        ...
        if any(kw in last_content for kw in ["hello", "hi ", "greet"]):
            return f"{prefix}Hello! I'm Genie, your AI platform assistant. How can I help you today?"
